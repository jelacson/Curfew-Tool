export const drawRectAndLabelFace = (descriptions, faceDB, participants, ctx) => {
    console.log("faceDB is type of", typeof faceDB);
    ...
    descriptions &&
        descriptions.forEach((desc) => {
            ...
            console.log("landmarksPoint", landmarksPoint)
            console.log("faceDB", faceDB);
            // const text = desc['class'];

            const bestMatch = faceDB.findBestMatch(desc.descriptor);

            if (faceDB !== null && typeof faceDB === 'object' && typeof faceDB.findBestMatch === 'function') {
                const bestMatch = faceDB.findBestMatch(desc.descriptor);

                console.log("FOUND", bestMatch._label)
