...
export async function createMatcher(faceDB, maxDescriptorDistance) {
    try {
        // Create labeled descriptors of member from profile
        if (faceDB !== null) {
            let labeledDescriptors = faceDB.map(
                (profile) =>
                    new faceapi.LabeledFaceDescriptors(
                        profile.studentId,
                        [new Float32Array(profile.faceDescriptor.match(/-?\d+(?:\.\d+)?/g).map(Number))],
                    )
            );
